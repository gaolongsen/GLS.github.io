...
3. **Update Rule**:
   ...
   - ![https://latex.codecogs.com/svg.image?x_k - \hat{x}_{1|k-1}](https://latex.codecogs.com/svg.image?x_k - \hat{x}_{1|k-1}) is the prediction error at time $$k$$ using the estimate from time $$k-1$$.
...
1. **Prediction**:
   ...
   - Compute the prediction error ![](https://latex.codecogs.com/svg.image?\tilde{x}_{1|k}) based on the true parameter $$\theta^*$$.
...
2. **Update**:
   ...
   - The term ![](https://latex.codecogs.com/svg.image?a_1\mu&space;D(x_{k-1},u_{k-1})^\top(x_k-\hat{x}_{1|k-1}))adjusts the parameter estimate in the direction that reduces the prediction error.
