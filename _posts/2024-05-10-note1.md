...
- For a more compact notation in the following proposition, several definitions are made:
  - $$d_{l|k}^j = A_0 x_{l|k}^j + B_0 u\_{l|k}^j - z\_{l+1|k}$$ and $$D_{l|k}^j = D(x_{l|k}^j, u_{l|k}^j)$$
    - $$x_{l|k}^j = z_{l|k} + \alpha_{l|k} x^j$$
    - $$u_{l|k}^j = u_{l|k}(x_{l|k}^j)$$
...
1. **Polytope and Vertices**:
   - The polytope $$\mathcal{X}_0$$ is defined by linear inequalities, and its shape is characterized by its vertices $$x^1, x^2, \ldots, x^v$$.

2. **Minkowski Sum Representation**:
   - The set $$\mathcal{X}_{l|k}$$ is described as a Minkowski sum, which combines a central point $$z_{l|k}$$ and a scaled version of the original polytope $$\mathcal{X}_0$$.

3. **Linear Constraints**:
   - The representation of $$\mathcal{X}_{l|k}$$ in both vertex and half-space form allows for the prediction constraints to be expressed as linear constraints. This is advantageous for solving the optimization problem efficiently.
