...
2. **Prediction Error**:
   ...
   -   ![](https://latex.codecogs.com/svg.image?\tilde{x}_{1|k}) is the error between the true next state (using the true parameter $$\theta^*$$) and the predicted state (using the estimated parameter ![](https://latex.codecogs.com/svg.image?\hat{\theta}_k&space;)).
...
**Eq.(14**) from the paper discusses a method for parametrizing a set of states ![](https://latex.codecogs.com/svg.image?\mathcal{X}_{l|k}) in the context of Model Predictive Control (MPC). This parametrization is based on using polytopes and can be leveraged to reformulate prediction constraints as linear constraints, which is essential for efficient MPC implementation.

#### Key Concepts and Notation

1. **Polytope $$\mathcal{X}_0$$**:
   
   - ![](https://latex.codecogs.com/svg.image?z_{N\mid&space;k}=\left\{z_{l\mid&space;k}\right\}_{l\in\mathbb{N}_0^N},z_{l\mid&space;k}\in\mathbb{R}^n): This is a polytope in $$\mathbb{R}^n$$ defined by a set of linear inequalities.
   - Vertices ![](https://latex.codecogs.com/svg.image?\alpha_{N\mid&space;k}=\left\{\alpha_{l\mid&space;k}\right\}_{l\in\mathbb{N}_0^N},\alpha_{l\mid&space;k}\in\mathbb{R}_{\geq&space;0}): The polytope $$\mathcal{X}_0$$ is characterized by its vertices $$x^1, x^2, \ldots, x^v$$.
   
2. **MPC Decision Variables**:
   
   -  $$z_\{N|k\} = \{ z_\{l|k\} \}_\{l \in \mathbb{N}_0^N\}, z_\{l|k\} \in \mathbb{R}^n$$: These are the decision variables for MPC at each time step $$l$$ given the current time step $$k$$.
   - $$\alpha_{N|k} = \{ \alpha_{l|k} \}_{l \in \mathbb{N}_0^N}, \alpha_{l|k} \in \mathbb{R}_{\geq 0}$$: These are scaling factors for the polytopes at each time step $$l$$ given the current time step $$k$$.
   
3. **State Set Parametrization**:
   - $$\mathcal{X}_{l|k}$$: The set of possible states at time step $$l$$ given the current time step $$k$$.
   - The parametrization of $$\mathcal{X}_{l|k}$$ is defined as:

     $$
     \mathcal{X}_{l|k} = \{ z_{l|k} \} \oplus \alpha_{l|k} \mathcal{X}_0
     $$
     This represents the Minkowski sum of $$ z_{l|k} $$ and a scaled version of the polytope $$\mathcal{X}_0$$, where:
   
     $$
     \mathcal{X}_{l|k} = \{ x \in \mathbb{R}^n \mid H_x (x - z_{l|k}) \leq \alpha_{l|k} 1 \}
     $$
   
     $$
     = \{ z_{l|k} \} \oplus \alpha_{l|k} \text{co} \{ x^1, x^2, \ldots, x^v \}
     $$
   
     
   
   - $$\text{co}\{ x^1, x^2, \ldots, x^v \}$$ denotes the convex hull of the vertices $$$$x^1, x^2, \ldots, x^v$$$$.

#### Reason for the Tubes Parametrization

- **Explicit Description**: The chosen parametrization provides an explicit description of $$\mathcal{X}_{l|k}$$ in both vertex and half-space form.
- **Reformulation of Constraints**: This explicit description can be exploited to reformulate the prediction constraints (referred to by equation (5) in the text) as linear constraints. This reformulation is crucial for incorporating the online updated parameter set $$\Theta_k$$.

#### Compact Notation

- For a more compact notation in the following proposition, several definitions are made:
  - $$d_{l|k}^j = A_0 x_{l|k}^j + B_0 u\_{l|k}^j - z\_{l+1|k}$$ and $$D_{l|k}^j = D(x_{l|k}^j, u_{l|k}^j)$$
    - ![](https://latex.codecogs.com/svg.image?x_{l|k}^j=z_{l|k}&plus;\alpha_{l|k}x^j&space;)
    - ![$$u_{l|k}^j = u_{l|k}(x_{l|k}^j)$$](https://latex.codecogs.com/svg.image?u_{l|k}^j=u_{l|k}(x_{l|k}^j))
  - $$\left[ \bar{w} \right]_i = \max_{w \in \mathcal{W}} [H_x] w$$ for all $$i \in \mathcal{N}_1^c$$
  - $$\left[ \bar{f} \right]_i = \max_{x \in \mathcal{X}_0} [F + GK] x$$ for all $$i \in \mathcal{N}_1^c$$

#### Detailed Explanation

1. **Polytope and Vertices**:
   - The polytope ![](https://latex.codecogs.com/svg.image?\mathcal{X}_{l|k}) is defined by linear inequalities, and its shape is characterized by its vertices $$x^1, x^2, \ldots, x^v$$.

2. **Minkowski Sum Representation**:
   - The set ![](https://latex.codecogs.com/svg.image?\mathcal{X}_{l|k}) is described as a Minkowski sum, which combines a central point ![](https://latex.codecogs.com/svg.image?z_{l|k}) and a scaled version of the original polytope ![](https://latex.codecogs.com/svg.image?\mathcal{X}_0&space;).

3. **Linear Constraints**:
   - The representation of ![](https://latex.codecogs.com/svg.image?\mathcal{X}_{l|k}) in both vertex and half-space form allows for the prediction constraints to be expressed as linear constraints. This is advantageous for solving the optimization problem efficiently.

4. **Parameter Update and Control Law**:
   - The compact notation simplifies the expressions and calculations involved in updating the parameters and applying the control law.
   - The terms ![](https://latex.codecogs.com/svg.image?d_{l|k}^j&space;) and ![](https://latex.codecogs.com/svg.image?D_{l|k}^j&space;) involve the system matrices $$A_0$$ and $$B_0$$, the state and input vectors, and the prediction of the next state ![](https://latex.codecogs.com/svg.image?z_{l&plus;1|k}).

5. **Optimization**:
   - The maximization terms $$\left[ \bar{w} \right]_i$$ and $$\left[ \bar{f} \right]_i$$ involve finding the worst-case effects of disturbances and the system's response, respectively. These terms ensure that the control design is robust to uncertainties.

### Conclusion

The provided text describes a sophisticated method for parametrizing the state set ![](https://latex.codecogs.com/svg.image?\mathcal{X}_{l|k}) in the context of MPC. By using polytopes and their vertices, the method enables the reformulation of prediction constraints as linear constraints, which is essential for efficient online optimization. The compact notation and definitions streamline the process of updating parameters and applying the control law, ensuring robust performance in the presence of uncertainties.
